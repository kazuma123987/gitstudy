...
{
    GLfloat z = radius * std::cos(PI * u);
    GLfloat x = radius * std::sin(PI * u) * std::cos(2 * PI * v);
    GLfloat y = radius * std::sin(PI * u) * std::sin(2 * PI * v);
...
void createSphere(GLfloat *sphere, GLfloat radius, GLuint Longitude, GLuint Latitude)
{
    ...
    { // 纬线u
        ...
        { // 经线v
            ...
            glm::vec3 point1 = getPoint(radius, uv1.x, uv1.y);
            glm::vec3 point2 = getPoint(radius, uv2.x, uv2.y);
            glm::vec3 point3 = getPoint(radius, uv3.x, uv3.y);
            glm::vec3 point4 = getPoint(radius, uv4.x, uv4.y);
...
uint8_t Game::init(void)
{
    ...
    this->imgui_init();  // imgui初始化
    this->global_init(); // 全局变量初始化

    // 设置纹理y轴(V轴)翻转
    stbi_set_flip_vertically_on_load(true);
    return 0;
}
uint8_t Game::glfw_init(void)
{
    glfwInit();
...
void Game::global_init()
{
    // Camera
    camera = new Camera(0.1f, 200.0f);
    // Font
    SetCurrentDirectoryA(filePath);
    ...
    createSphere(sphereArray, 90.0f, longitude, latitude);
...
void Game::rendFPS(GLFWwindow *window)
{
    static int fps = 0;
    fps++;
