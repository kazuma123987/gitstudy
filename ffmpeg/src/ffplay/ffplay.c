...
static int packet_queue_get(PacketQueue *q, AVPacket *pkt, int block, int *serial)
{
    ...
    for (;;)
    {
        ...
        {
            SDL_CondWait(q->cond, q->mutex); // 如果PacketQuene没有数据，则此线程先进入休眠，等待条件唤醒
        }
    }
    SDL_UnlockMutex(q->mutex);
    return ret;
...
static Frame *frame_queue_peek_writable(FrameQueue *f)
{
    ...
    {
        SDL_CondWait(f->cond, f->mutex);  // 如果frameQuene的大小不足，则进入休眠(唤醒后大小不足继续休眠)
    }
    SDL_UnlockMutex(f->mutex);
...
static Frame *frame_queue_peek_readable(FrameQueue *f)
{
    ...
    while (f->size - f->rindex_shown <= 0 &&
           !f->pktq->abort_request)
    {
        SDL_CondWait(f->cond, f->mutex); // 如果frameQuene空了，则进入休眠(唤醒后空了继续休眠)
    }
    SDL_UnlockMutex(f->mutex);
...
/* open a given stream(音频、视频或者字幕). Return 0 if OK */
...
static int read_thread(void *arg)
{
    ...
    for (;;)
    {
                                                                                         ...
                                                                                         stream_has_enough_packets(is->subtitle_st, is->subtitle_stream, &is->subtitleq))))
        {
            ...
            SDL_CondWaitTimeout(is->continue_read_thread, wait_mutex, 10); // 任何一个PacketQuene满了线程进入休眠
...
int main(int argc, char **argv)
{
    ...
    if (!input_filename)
    {
        const char default_inputfile[] = "../res/tera.mp4";
        input_filename = av_strdup(default_inputfile);
