...
static unsigned int translateButton(unsigned int button)
{
    ...
    {
    case '1':
        return (1 << BTN_ACTION1);
    case '2':
        return (1 << BTN_ACTION2);
    case '3':
        return (1 << BTN_ACTION3);
    case '4':
        return (1 << BTN_ACTION4);
    case VK_LEFT:
        return (1 << BTN_LEFT);
    case VK_RIGHT:
        return (1 << BTN_RIGHT);
...
{
    char *filePath = (char *)calloc(FILENAME_MAX, sizeof(char));
    // GetCurrentDirectoryA(FILENAME_MAX, filePath);
    GetModuleFileNameA(NULL, filePath, FILENAME_MAX);
    size_t fileLen = strlen(filePath);
    const uint32_t tmp_size = sizeof("fmod_api")-1;
    for (size_t i = fileLen; i> tmp_size && strncmp(&filePath[i-tmp_size], "fmod_api", tmp_size) != 0; i--)
    {
        filePath[i] = '\0';
    }

    strcat(filePath, "core/examples/media/");
