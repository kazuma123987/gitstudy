...
void press_3D_input(GLFWwindow *window, bool *flag_3D, clock_t *pre)
{
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window)) // 渲染循环
    {
        ...
        if (cur_t-pre_choose_t>200)
        {
            press_choose_input(window, &system, &sound2, &channel2,&pre_choose_t);
        }
        bool flag_3D, pre;
        pre = flag_3D;
        ...
        /*渲染部分*/
        glClearColor(0.1, 0.2, 0.3, 0.4); // 设置运行glclear时的颜色，这是状态参数,分别代表红、绿、蓝、alpha
